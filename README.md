...
Поскольку модель является лёгкой, самым простым способом проверки ее работы явялется непосредственный запуск блокнота (параметры модели не сохранялись отдельно).
