...
Для воспроизведения результатов нужно использовать библиотеки jaxopt (использовалась версия 0.8.3) и jax. В случае Google Collab библиотека jax предустановлена и из коробки работает с видеокартой. В случае Yandex DataSphere библиотека также предустановлена, но для работы с видеокартой нужно установить ее отдельно (см. https://jax.readthedocs.io/en/latest/installation.html и notebooks/JAX_bloch_wall_nn.ipynb).

Для сохранения результатов использовалась библиотека numpy.

### Примечание

Поскольку модель является лёгкой, самым простым способом проверки ее работы явялется непосредственный запуск блокнота (параметры модели не сохранялись отдельно). Время работы цикла обучения на 1000 эпох составляет приблизительно 5 минут (0.3 секунды на эпоху).
